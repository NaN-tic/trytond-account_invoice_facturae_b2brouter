...
class Invoice(metaclass=PoolMeta):
    ...
    b2brouter_message = fields.Text('B2BRouter Message', readonly=True)
    ...
    def generate_facturae(self, certificate=None, service=None):
        ...
        if (self.invoice_date > Date.today() and
            ...
            raise UserError(gettext('account_invoice_facturae_b2brouter.'
                    'msg_error_send_b2brouter_future',
    ...
    def send_facturae_b2brouter(self):
        ...
        try:
            ...
            raise UserError(gettext(
                    'account_invoice_facturae_b2brouter.'
                    'msg_error_send_b2brouter_timeout',
                    invoice=self.rec_name,
                    error=str(err)))
            ...
            raise UserError(gettext(
                    'account_invoice_facturae_b2brouter.'
                    'msg_error_send_b2brouter_error',
                    invoice=self.rec_name,
                    error=str(err)))
    ...
    def b2brouter_delete_invoice(self):
        ...
        try:
            ...
            raise UserError(gettext(
                    'account_invoice_facturae_b2brouter.'
                    'msg_error_delete_b2brouter_timeout',
                    invoice=self.rec_name,
                    error=str(err)))
        except socket.error as err:
            _logger.warning(
            ...
            raise UserError(gettext(
                    'account_invoice_facturae_b2brouter.'
                    'msg_error_delete_b2brouter_error',
                    invoice=self.rec_name,
                    error=str(err)))
